    ...
    if (order_id) {
      ...
      if (status === "cancel" || status === "failure") {
        ...
        order.system_message?.push(
          `Bkash payment status is ${status} because of that order status is ${ORDER_STATUS.FAILED}`
        );

        order.order_status = ORDER_STATUS.FAILED as IOrderStatus;
        ...
        // );
        return res.redirect(
          `${process.env.LOCAL_CLIENT_URL}/checkout/payment/verify?order_id=${order.id}&message=${order.order_note}&status=${order.order_status}&paymentID=${paymentID}`
        );
      }
      if (status == "success") {
        const executeData = await BkashService.executePayment(
          paymentID as string
        );

        if (
          executeData &&
          (executeData as any).statusMessage === "Successful"
        ) {
          order.system_message?.push(
            `Bkash payment status is ${status} because of that order status is ${ORDER_STATUS.PLACED}`
          );

          order.order_status = ORDER_STATUS.PLACED as IOrderStatus;
          order.is_delivery_charge_paid = true;
          order.paid_amount = parseFloat((executeData as any)?.amount || "0");
          const transaction = await TransactionService.createTransaction({
            order: order.id,
            payment_date: new Date(Date.now()),
            payment_id: paymentID as string,
            trx_status: status,
            amount: order.payable_amount,
            trx_id: (executeData as any)?.trxID || "Not found",
            user: order.user || null,
            currency: "BDT",
            payment_by: order.user || "Not found",
            message: order.order_note,
          ...
          return res.redirect(
            `${process.env.LOCAL_CLIENT_URL}/checkout/payment/verify?order_id=${order.id}&message=${"Payment status success"}&status=${ORDER_STATUS.PLACED}&paymentID=${paymentID}`
          );
        } else {
          //  Update order status to failed and add order_note
          ...
          // todo change with the help of @Nahid vi
          return res.redirect(
            `${process.env.LOCAL_CLIENT_URL}/checkout/payment/verify?order_id=${order?.order_id}&message=${"Payment Cancel or Failure"}&status=${ORDER_STATUS.FAILED}&paymentID=${paymentID}`
          );
        }
      }
    }
