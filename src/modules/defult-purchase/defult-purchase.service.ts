  ...
  async createPurchase(data: IDefaultsPurchase): Promise<IDefaultsPurchase> {
    ...
    try {
      ...
      if (defaults) {
        ...
        // Create new document with provided _id generated by mongoose (or we can pre-generate)
        defaults = await DefaultsPurchaseModel.create(
          [
            {
              variant: variantId,
              product: productId,
      ...
      // rethrow after abort so caller can handle
      throw error;
    } finally {
      session.endSession();
    }
